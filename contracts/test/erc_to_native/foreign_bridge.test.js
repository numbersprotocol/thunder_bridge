const ForeignBridge = artifacts.require("ForeignBridgeERC677ToNative.sol");
...
contract('ForeignBridge_ERC677_to_NATIVE', async (accounts) => {
  ...
  describe('#initialize', async () => {
    it('should initialize', async () => {
      ...
      const { logs } = await foreignBridge.initialize(validatorContract.address, token.address, requireBlockConfirmations, gasPrice, maxPerTx, homeDailyLimit, homeMaxPerTx, owner, FEE_PERCENT);
  ...
  describe('#executeSignatures with 2 minimum signatures', async () => {
    ...
    it('works with max allowed number of signatures required', async () => {
      ...
      const {
        receipt
      } = await foreignBridgeWithMaxSigs.executeSignatures(v, r, s, message).should.be.fulfilled
    ...
    it('Should fail if length of signatures is wrong', async () => {
      ...
      const {
        logs
      } = await foreignBridgeWithThreeSigs.executeSignatures(v, r, s, message).should.be.fulfilled
      logs[0].event.should.be.equal('RelayedMessage')
      logs[0].args.recipient.should.be.equal(recipient)
      logs[0].args.value.should.be.bignumber.equal(value)
      true.should.be.equal(await foreignBridgeWithThreeSigs.relayedMessages(txHash))
  ...
  describe('#onTokenTransfer', async () => {
    it('can only be called from token contract', async () => {
      ...
      const token = await ERC677BridgeToken.new('POA ERC20 Foundation', 'POA20', 18, { from: owner })
    ...
    it('should be able to specify a different receiver', async () => {
      ...
      token = await ERC677BridgeToken.new('POA ERC20 Foundation', 'POA20', 18, { from: owner })
      const foreignBridge = await ForeignBridge.new()
      await foreignBridge.initialize(validatorContract.address, token.address, requireBlockConfirmations, gasPrice, maxPerTx, homeDailyLimit, homeMaxPerTx, owner, feePercent);
      await token.mint(user, halfEther, { from: owner }).should.be.fulfilled
      await token.addBridgeContract(foreignBridge.address, {from: owner})
    ...
    it('transfer should trigger onTokenTransfer', async () => {
      ...
      expect(toBN(events[1].returnValues.value)).to.be.bignumber.equal(halfEther)
    })
  })
