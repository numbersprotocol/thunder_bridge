...
contract("HomeBridge_ERC20_to_ERC20", async accounts => {
  ...
  describe("#executeAffirmation", async () => {
    ...
    it("works with 5 validators and 3 required signatures with v2", async () => {
      const recipient = accounts[8];
      const authoritiesFiveAccs = [
        accounts[1],
        accounts[2],
      ...
      await minter.initialize(token.address, homeBridgeWithThreeSigs.address);
      await homeBridgeWithThreeSigs.initialize(
        validatorContractWith3Signatures.address,
        oneEther,
        halfEther,
      ...
      await homeBridgeWithThreeSigs.setMinter(minter.address);
