...
function abiSignature(web3, abi) {
  const signature =
    abi.name + "(" + abi.inputs.map(i => i.type).join(",") + ")";
  return Web3Utils.keccak256(signature);
}
// First matching event only
function eventFromReceipt(web3, r, event, contractAbi) {
...
contract("HomeBridge_ERC677_to_ERC677", async accounts => {
  ...
  describe("#initialize", async () => {
    ...
    it("sets variables", async () => {
      ...
      expect(await homeContract.validatorContract()).to.be.equal(
        validatorContract.address
      );
      ...
      expect(await homeContract.feePercent()).to.be.bignumber.equal(
        FEE_PERCENT
      );
    });
    ...
    it("can be deployed via upgradeToAndCall", async () => {
      ...
      let data = homeContract.contract.methods
        .initialize(
          validatorContract.address,
          "3",
          "2",
      ...
      await storageProxy.upgradeToAndCall("1", homeContract.address, data)
        .should.be.fulfilled;
      let finalContract = await HomeBridge.at(storageProxy.address);

      expect(await finalContract.isInitialized()).to.be.equal(true);
      expect(await finalContract.validatorContract()).to.be.equal(
        validatorContract.address
      );
  ...
  describe("#executeAffirmation", async () => {
    ...
    beforeEach(async () => {
      ...
      await token.mint(homeBridge.address, ether("100"));
    });
    it("should allow validator to withdraw", async () => {
      ...
      totalSupply.should.be.bignumber.equal(ether("100"));
    ...
    it("test with 2 signatures required", async () => {
      ...
      await token2sig.mint(homeBridgeWithTwoSigs.address, ether("100"));
      const recipient = accounts[5];
      const value = halfEther;
      const transactionHash =
        "0x806335163828a8eda675cff9c84fa6e6c7cf06bb44cc6ec832e42fe789d01415";
      ...
      expect(await token2sig.totalSupply()).to.be.bignumber.equal(ether("100"));
    ...
    it("works with 5 validators and 3 required signatures", async () => {
      ...
      await token.mint(homeBridgeWithThreeSigs.address, ether("100"));
  ...
  describe("#isAlreadyProcessed", async () => {
    it("returns ", async () => {
      ...
      const processedNumbers = [
        bn.add(toBN(1)).toString(10),
        bn.add(toBN(100)).toString(10)
      ];
      true.should.be.equal(
        await homeBridge.isAlreadyProcessed(processedNumbers[0])
      );
      true.should.be.equal(
        await homeBridge.isAlreadyProcessed(processedNumbers[1])
      );
  ...
  describe("#submitSignature", async () => {
    ...
    it("allows a validator to submit a signature", async () => {
      ...
      const { logs } = await homeBridgeWithTwoSigs.submitSignature(
        signature,
        message,
        { from: authorities[0] }
      ).should.be.fulfilled;
      ...
      const signatureFromContract = await homeBridgeWithTwoSigs.signature(
        msgHashFromLog,
        0
      );
      const messageFromContract = await homeBridgeWithTwoSigs.message(
        msgHashFromLog
      );
      ...
      "1".should.be.bignumber.equal(
        await homeBridgeWithTwoSigs.numMessagesSigned(hashMsg)
      );
      const hashSenderMsg = Web3Utils.soliditySha3(authorities[0], hashMsg);
      true.should.be.equal(
        await homeBridgeWithTwoSigs.messagesSigned(hashSenderMsg)
      );
    });
    it("when enough requiredSignatures are collected, CollectedSignatures event is emitted", async () => {
      ...
      "2".should.be.bignumber.equal(
        await validatorContractWith2Signatures.requiredSignatures()
      );
      await homeBridgeWithTwoSigs.submitSignature(signature, message, {
        from: authoritiesTwoAccs[0]
      }).should.be.fulfilled;
      await homeBridgeWithTwoSigs
        .submitSignature(signature, message, { from: authoritiesTwoAccs[0] })
        .should.be.rejectedWith(ERROR_MSG);
      await homeBridgeWithTwoSigs
        .submitSignature(signature, message, { from: authoritiesTwoAccs[1] })
        .should.be.rejectedWith(ERROR_MSG);
      const { logs } = await homeBridgeWithTwoSigs.submitSignature(
        signature2,
        message,
      ...
      logs[1].args.authorityResponsibleForRelay.should.be.equal(
        authoritiesTwoAccs[1]
      );
      const processed = toBN(2)
        .pow(toBN(255))
        .add(toBN(2));
      processed.should.be.bignumber.equal(
        await homeBridgeWithTwoSigs.numMessagesSigned(hashMsg)
      );
    });
    it("works with 5 validators and 3 required signatures", async () => {
      ...
      "3".should.be.bignumber.equal(
        await validatorContractWith3Signatures.requiredSignatures()
      );
      ...
      const { logs } = await homeBridgeWithThreeSigs.submitSignature(
        signature3,
        message,
        { from: authoritiesFiveAccs[2] }
      ).should.be.fulfilled;
    ...
    it("attack when increasing requiredSignatures", async () => {
      ...
      const attackerTx = await homeBridgeWithTwoSigs
        .submitSignature(signature3, message, { from: authoritiesTwoAccs[2] })
        .should.be.rejectedWith(ERROR_MSG);
    });
  ...
  describe("#fixAssetsAboveLimits", async () => {
    ...
    beforeEach(async () => {
      ...
      await storageProxy.upgradeTo("1", homeBridgeImpl.address).should.be
        .fulfilled;
    ...
    it("Should reduce outOfLimitAmount and not emit any event", async () => {
      ...
      } = await homeBridge.executeAffirmation(
        recipient,
        value,
        transactionHash,
        { from: authorities[0] }
      ...
      const { logs } = await homeBridge.fixAssetsAboveLimits(
        transactionHash,
        false
      ).should.be.fulfilled;
    ...
    it("Should not be allow to be called by an already fixed txHash", async () => {
      ...
      await homeBridge.fixAssetsAboveLimits(transactionHash, false).should.be
        .fulfilled;
      ...
      await homeBridge
        .fixAssetsAboveLimits(transactionHash, false)
        .should.be.rejectedWith(ERROR_MSG);
      await homeBridge.fixAssetsAboveLimits(transactionHash2, false).should.be
        .fulfilled;
      ...
      await homeBridge
        .fixAssetsAboveLimits(transactionHash2, false)
        .should.be.rejectedWith(ERROR_MSG);
    });
    it("Should fail if txHash didnt increase out of limit amount", async () => {
      ...
      await homeBridge
        .fixAssetsAboveLimits(invalidTxHash, true)
        .should.be.rejectedWith(ERROR_MSG);
    });
  ...
  describe("#callToken", async () => {
    it("Mint token from bridge", async () => {
      ...
      let args = web3.eth.abi.encodeFunctionCall(funcAbi, [
        mintReceiver,
        pointOneEther.toString()
      ]);
  ...
  describe("#Fee", async () => {
    it("Calculate fee as excepted", async () => {
      ...
      const { logs: logsDFP } = await homeBridge.setDepositFeePercent(
        fivePercent
      );
      ...
      const { logs: logsWFP } = await homeBridge.setWithdrawFeePercent(
        fivePercent
      );
    ...
    it("test with 2 signatures and fee", async () => {
      ...
      balanceAfter.should.be.bignumber.equal(
        balanceBefore.add(value).sub(feeValue)
      );
      const feeReceived = await token2sig.balanceOf(feeReceiver);
      feeReceived.should.be.bignumber.equal(feeValue);

      const markedAsProcessed = await homeBridgeWithTwoSigs.numAffirmationsSigned(
        msgHash
      );
      const processed = toBN(2)
        .pow(toBN(255))
      ...
      const {
        receipt,
        logs
      } = await token2sig.transfer(
        homeBridgeWithTwoSigs.address,
        balanceAfter,
        { from: recipient }
      );

      const e = eventFromReceipt(
        web3,
        receipt,
        "UserRequestForSignature",
        homeBridgeWithTwoSigs.abi
      );
    ...
    it("only owner can set fee percent", async () => {
      ...
      await homeContract
        .setWithdrawFixedFee(newFeePercent, { from: accounts[5] })
        .should.be.rejectedWith(ERROR_MSG);

      await homeContract
        .setFeePercent(newFeePercent, { from: accounts[5] })
        .should.be.rejectedWith(ERROR_MSG);

      // deprecated old fee percent
      await homeContract
        .setFeePercent(newFeePercent)
        .should.be.rejectedWith("Deprecated");
    });
  });
});
